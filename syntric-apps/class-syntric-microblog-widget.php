...
class Syntric_Microblog_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts->have_posts() ) :
			...
			endwhile;
			echo $tab . '<li class="widget-item nav-item">' . $lb;
			echo $tab . $tab . '<a href="' . get_term_link( $term->term_id ) . '" class="widget-entry nav-link widget-more-link">' . $lb;
			echo $tab . $tab . $tab . 'more posts';
			echo $tab . $tab . '</a>';
			echo $tab . '</li>' . $lb;
		else :
			echo $tab . '<li class="widget-item nav-item">' . $lb;
		...
		if ( is_user_logged_in() && ( current_user_can( 'administrator' ) || current_user_can( 'editor' ) || $post->post_author == get_current_user_id() ) ) {
			// todo: put an new post button here, for logged in only.  admin, editor or teacher/owner can post
		}
	}
