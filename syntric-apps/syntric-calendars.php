<?php
	 ...
	 */
	add_action( 'init', 'syntric_register_syn_calendar' );
	function syntric_register_syn_calendar() {
		$labels = [
			'name'                  => _x( 'Syn Calendars', 'syntric' ),
			'singular_name'         => _x( 'Syn Calendar', 'syntric' ),
			'menu_name'             => _x( 'Syn Calendars', 'syntric' ),
			'name_admin_bar'        => _x( 'Syn Calendar', 'syntric' ),
			'add_new'               => __( 'Add New Syn Calendar', 'syntric' ),
			'add_new_item'          => __( 'Add New Syn Calendar', 'syntric' ),
			'new_item'              => __( 'New Syn Calendar', 'syntric' ),
			'edit_item'             => __( 'Edit Syn Calendar', 'syntric' ),
			'view_item'             => __( 'View Syn Calendar', 'syntric' ),
			'all_items'             => __( 'All Syn Calendars', 'syntric' ),
			'search_items'          => __( 'Search Syn Calendars', 'syntric' ),
			'parent_item_colon'     => __( 'Parent Syn Calendars:', 'syntric' ),
			'not_found'             => __( 'No syn calendars found.', 'syntric' ),
			'not_found_in_trash'    => __( 'No syn calendars found in trash.', 'syntric' ),
			'archives'              => _x( 'Syn calendar archives', 'syntric' ),
			'insert_into_item'      => _x( 'Insert into syn calendar', 'syntric' ),
			'uploaded_to_this_item' => _x( 'Uploaded to this syn calendar', 'syntric' ),
			'filter_items_list'     => _x( 'Filter syn calendars list', 'syntric' ),
			'items_list_navigation' => _x( 'Syn calendars list navigation', 'syntric' ),
			'items_list'            => _x( 'Syn calendars list', 'syntric' ),
		];
		$args   = [
			'labels'            => $labels,
			'public'            => true,
	 ...
	 */
	add_action( 'init', 'syntric_register_syn_event' );
	function syntric_register_syn_event() {
		$labels = [
			'name'                  => _x( 'Syn Events', 'syntric' ),
			'singular_name'         => _x( 'Syn Event', 'syntric' ),
			'menu_name'             => _x( 'Syn Events', 'syntric' ),
			'name_admin_bar'        => _x( 'Syn Event', 'syntric' ),
			'add_new'               => __( 'Add Syn New', 'syntric' ),
			'add_new_item'          => __( 'Add Syn New Event', 'syntric' ),
			'new_item'              => __( 'New Syn Event', 'syntric' ),
			'edit_item'             => __( 'Edit Syn Event', 'syntric' ),
			'view_item'             => __( 'View Syn Event', 'syntric' ),
			'all_items'             => __( 'All Syn Events', 'syntric' ),
			'search_items'          => __( 'Search Syn Events', 'syntric' ),
			'parent_item_colon'     => __( 'Parent Syn Events:', 'syntric' ),
			'not_found'             => __( 'No syn events found.', 'syntric' ),
			'not_found_in_trash'    => __( 'No syn events found in trash.', 'syntric' ),
			'archives'              => _x( 'Syn event archives', 'syntric' ),
			'insert_into_item'      => _x( 'Insert into syn event', 'syntric' ),
			'uploaded_to_this_item' => _x( 'Uploaded to this syn event', 'syntric' ),
			'filter_items_list'     => _x( 'Filter syn events list', 'syntric' ),
			'items_list_navigation' => _x( 'Syn events list navigation', 'syntric' ),
			'items_list'            => _x( 'Syn events list', 'syntric' ),
		];
		$args   = [
			'labels'              => $labels,
			'public'              => false,
	...
	//add_action( 'restrict_manage_posts', 'syntric_calendar_restrict_manage_posts', 10 );
	function syntric_calendar_restrict_manage_posts() {
		global $post_type;
		if( is_admin() && $post_type == 'syn_event' ) {
			...
			$calendar_ids    = syntric_get_calendar_ids();
	...
	function ___syntric_save_calendar( $post_id ) {
		global $pagenow;
	...
	function syntric_sync_calendar( $args ) {
		slog( 'sync calendar triggered' );
		$calendar_id = $args[ 'post_id' ];
		//$post_type   = ( isset( $args[ 'post_type' ] ) ) ? $args[ 'post_type' ] : '';
		$force_sync       = ( isset( $args[ 'force_sync' ] ) ) ? $args[ 'force_sync' ] : false;
		$sync_back        = ( isset( $args[ 'sync_back' ] ) ) ? $args[ 'sync_back' ] : false;
		$sync_back_months = ( isset( $args[ 'sync_back' ] ) ) ? $args[ 'sync_back' ] : false;
		$log              = '';
		$lb               = "\n";
		$log              .= 'Sync started with values ' . $lb . print_r( $args, true );
		$log              .= 'Sync type: ' . ( $force_sync ) ? 'manual sync' : 'scheduled sync';
		$log              .= 'Sync back: ' . ( $sync_back ) ? 'yes' : 'no';
		if( $sync_back && $sync_back_months ) {
		...
		update_field( 'syn_calendar_last_sync_result', $log, $calendar_id );
		$calendar_events = syn_get_calendar_events( $calendar_id, null, 'all', - 1, 'ID' );
		update_field( 'syn_calendar_event_count', count( $calendar_events ), $calendar_id );
		
		//syntric_write_calendar_json_file( $calendar_id );
		return true;
	}
	 ...
	 */
	add_action( 'before_delete_post', 'syntric_before_delete_calendar' );
	function syntric_before_delete_calendar( $post_id ) {
		...
		if( 'syn_calendar' == $post -> post_type ) {
			...
			$event_ids = syntric_get_calendar_events( $post_id, null, 'all', - 1, 'ids' );
			if( $event_ids ) {
	 ...
	 */
	add_filter( 'pre_get_posts', 'syntric_calendar_pre_get_posts', 1, 1 );
	function syntric_calendar_pre_get_posts( $query ) {
		global $pagenow;
			...
			if( $query -> get( 'post_type' ) == 'syn_event' || $query -> get( 'post_type' ) == 'syn_calendar' ) {
				...
				if( isset( $_GET[ 'orderby' ] ) ) {
					...
					if( $query -> get( 'post_type' ) == 'syn_event' ) {
						...
						// todo: check if the alternate unset structure works
	...
	function syntric_purge_calendar( $post_id ) {
		$events = syntric_get_calendar_events( [
			$post_id,
			null,
			'all',
			- 1,
	...
	function syntric_get_posts_with_calendar_widget() {
		$args  = [
			'numberposts' => - 1,
			'post_type'   => [
				'page',
	...
	function syntric_save_page_calendar( $post_id ) {
		$post_id         = syntric_resolve_post_id( $post_id );
		...
		if( $calendar_active ) {
			...
			if( 0 == $_calendar_id ) {
				...
				if( $calendar_id ) {
					...
					if( $sync ) {
						syntric_schedule_calendar_sync( $calendar_id );
					}
	...
	function _____save_this_for_calendar_widget____syntric_display_calendar( $initial = true, $echo = true ) {
    ...
    <tr>';
		...
		if( $echo ) {
			 ...
			 */
			echo apply_filters( 'syntric_display_calendar', $calendar_output );
			
			return;
		}
		
		/** This filter is documented in wp-includes/general-template.php */
		return apply_filters( 'syntric_display_calendar', $calendar_output );
	}