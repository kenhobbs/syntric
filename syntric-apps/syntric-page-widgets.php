	...
	if ( $calendar_active ) {
		...
		if ( 0 == $_calendar_id ) {
			...
			$calendar_args = array(
				...
				'post_name'   => $page->post_name . '-calendar',
			...
			if ( $calendar_id ) {
				...
				$id             = get_field( 'syn_new_calendar_id', $page_id );
				$sync_now       = get_field( 'syn_new_calendar_sync_now', $page_id );
				$sync           = get_field( 'syn_new_calendar_sync', $page_id );
				$sync_frequency = get_field( 'syn_new_calendar_sync_frequency', $page_id );
				$sync_range     = get_field( 'syn_new_calendar_sync_range', $page_id );
				$delete_events  = get_field( 'syn_new_calendar_delete_events', $page_id );
				$delete_after   = get_field( 'syn_new_calendar_delete_after', $page_id );
				...
				if ( $sync_now ) {
					syn_sync_calendar( array( 'post_id' => $calendar_id, 'post_type' => 'syn_calendar' ) );
				}
				// clear out all the widget values
				update_field( 'syn_new_calendar_id', '', $page_id );
				update_field( 'syn_new_calendar_sync_now', 0, $page_id );
				update_field( 'syn_new_calendar_sync', 0, $page_id );
				update_field( 'syn_new_calendar_sync_frequency', null, $page_id );
				update_field( 'syn_new_calendar_sync_range', null, $page_id );
				update_field( 'syn_new_calendar_delete_events', null, $page_id );
				update_field( 'syn_new_calendar_delete_after', null, $page_id );
			}
		}
	}
}
	...
	if ( $microblog_active ) {
		...
		if ( $new_microblog_post ) {
			...
			$post_args         = array(
				...
				'tax_input'     => array( 'microblog' => array( $term->term_id ) ),
			);
