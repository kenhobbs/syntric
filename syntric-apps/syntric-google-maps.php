<?php
	 ...
	 */
	add_action( 'wp_enqueue_scripts', 'syntric_google_maps_enqueue_scripts' );
	function syntric_google_maps_enqueue_scripts() {
		$google_api_key = get_field( 'syn_google_api_key', 'option' );
		if( have_rows( 'syntric_google_maps', 'option' ) && $google_api_key ) {
	...
	add_action( 'wp_ajax_nopriv_syntric_fetch_map', 'syntric_fetch_map' );
	add_action( 'wp_ajax_syntric_fetch_map', 'syntric_fetch_map' );
	function syntric_fetch_map() {
		check_ajax_referer( 'syntric_fetch_map' );
		if( have_rows( 'syntric_google_maps', 'option' ) ) {
			while( have_rows( 'syntric_google_maps', 'option' ) ) : the_row();
