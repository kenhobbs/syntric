...
function syn_register_microblog_taxonomy() {
	...
	$tax_args = array(
		...
		'rewrite'            => array( 'slug' => 'microblog', 'with_front' => true ),
		'query_var'          => false,
	);
	...
	if ( 'microblog' == $taxonomy ) {
		...
		if ( $uncat_is_default ) {
			if ( $news_exists ) {
				...
				$news_posts = get_posts( array( 'numberposts' => - 1, 'category' => $news_term_tax_ids[ 'term_id' ] ) );
				if ( $news_posts ) {
					foreach ( $news_posts as $nwsp ) {
						$set_ret = wp_set_post_categories( $nwsp->ID, array( 1 ), false );
			...
			// rename default
			$upd_ret = wp_update_term( 1, 'category', array(
				'name'   => 'News',
				'slug'   => 'news',
				'parent' => 0,
			) );
			...
			$other_term_ = wp_insert_term( $other_term[ 'name' ], 'category', array(
			...
			$other_posts = get_posts( array( 'numberposts' => - 1, 'category' => 1 ) );
			if ( $other_posts ) {
				$cat_arr   = array();
	...
	if ( is_admin() && is_numeric( $post_id ) && 'page' == $post->post_type && isset( $_REQUEST[ 'acf' ] ) && ! wp_is_post_revision( $post->ID ) ) {
		...
		if ( $microblog_active ) {
			if ( category_exists( 'Microblogs' ) ) {
				...
				$cat_id = wp_insert_category( array( 'cat_name' => 'Microblogs' ) );
			}
			...
			if ( term_exists( $slugs, 'microblog' ) ) {
				...
				$tax_term_ids = wp_insert_term( $titles, 'microblog', array(
					'slug'        => $slugs,
