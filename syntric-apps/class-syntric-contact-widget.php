...
class Syntric_Contact_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( 'person' == $contact_type ) {
			...
			if ( $user_id ) {
				...
				if ( $user && $user instanceof WP_User ) {
					...
					$contact .= $tab . '<div class="contact-entry d-flex flex-column">' . $lb;
					...
					if ( in_array( 'email', $include_fields ) && $email ) :
						$contact .= $tab . $tab . '<span class="entry-email">' . $lb;
						$contact .= $tab . $tab . $tab . '<a href="mailto:' . antispambot( $email, true ) . '" class="contact-email" title="Email">' . antispambot( $email ) . '</a>' . $lb;
		...
		//$_include_fields = get_field( 'syn_contact_include_person_fields', $post->ID );
		//$_include_fields = get_field( 'syn_contact_widget_include_organization_fields', 'widget_' . $args['widget_id'] );
		...
		/*switch ( $contact_type ) :
			case 'person' :
				...
				if ( in_array( 'title', $include_fields ) && $title_ ) :
					echo $tab . $tab . '<span class="entry-title">' . $title_ . '</span>' . $lb;
				endif;
				if ( in_array( 'email', $include_fields ) && $email ) :
					echo $tab . $tab . '<span class="entry-email">' . $lb;
					echo $tab . $tab . $tab . '<a href="mailto:' . antispambot( $email, true ) . '" class="contact-email" title="Email">' . antispambot( $email ) . '</a>' . $lb;
			...
			case 'organization' :
				...
				if ( in_array( 'logo', $include_fields ) ) {
					if ( $logo ) :
						...
						echo $tab . '<span class="entry-image">' . $lb;
						if ( in_array( 'url', $include_fields ) ) :
							echo $tab . $tab . '<a href="' . $url . '" class="entry-logo-link" title="Go to ' . $organization . '">' . $lb;
						endif;
						echo $tab . $tab . $tab . '<img src="' . $logo[ 'url' ] . '" class="entry-logo" alt="' . $organization . ' Logo">' . $lb;
						if ( in_array( 'url', $include_fields ) ) :
							echo $tab . $tab . '</a>' . $lb;
						endif;
						echo $tab . '</span>' . $lb;
				...
				echo $tab . '<div class="contact-entry d-flex flex-column">' . $lb;
				// name
				echo $tab . $tab . '<span class="entry-name">' . $organization . '</span>' . $lb;
				// address
				if ( in_array( 'address', $include_fields ) ) :
					if ( $address ) :
						echo $tab . $tab . '<span class="entry-address">' . $address . '</span>' . $lb;
						if ( ! empty( $address_2 ) ) :
							echo $tab . $tab . '<span class="entry-address-2">' . $address_2 . '</span>' . $lb;
						endif;
					endif;
					if ( $city || $state || $zip_code ) :
						echo $tab . $tab . '<span class="d-flex flex-row">' . $lb;
						if ( ! empty( $city ) ) :
							echo $tab . $tab . $tab . '<span class="entry-city">' . $city . '</span>' . $lb;
						endif;
						if ( ! empty( $state ) ) :
							echo $tab . $tab . $tab . '<span class="entry-state">' . $state . '</span>' . $lb;
						endif;
						if ( ! empty( $zip_code ) ) :
							echo $tab . $tab . $tab . '<span class="entry-zip-code">' . $zip_code . '</span>' . $lb;
						endif;
						echo $tab . $tab . '</span>' . $lb;
					endif;
				endif;
				...
				if ( in_array( 'email', $include_fields ) ) :
					if ( $email ) :
						echo $tab . $tab . '<a href="mailto:' . antispambot( $email, true ) . '" class="entry-email" title="Email">' . antispambot( $email ) . '</a>' . $lb;
					endif;
				endif;
				// phone
				if ( in_array( 'phone', $include_fields ) ) :
				...
				if ( in_array( 'fax', $include_fields ) ) :
					if ( $fax ) :
						echo $tab . $tab . '<span class="entry-phone">' . $fax . ' fax</span>' . $lb;
					endif;
				endif;
				// url
				if ( in_array( 'url', $include_fields ) ) :
					if ( $url ) :
						echo $tab . $tab . '<a href="' . $url . '" class="entry-url" title="Go to ' . get_sub_field( 'name' ) . '">' . $url_display . '</a>' . $lb;
					endif;
				endif;
				echo '</div>' . $lb;
				if ( in_array( 'logo', $include_fields ) ) {
