<?php
	...
	/*function ___syntric_save_post( $post_id ) {
		global $pagenow;
		...
		switch( $post -> post_type ) {
			...
			case 'page' :
				...
				switch( $page_template ) {
					case 'class':
						//$class = syntric_get_teacher_class_page();
						//$class = syntric_get_class_page_content();
	...
	function syntric_get_excerpt_badges( $post_id = null ) {
		...
		switch( $post -> post_type ) {
			case 'syntric_event' :
				...
				break;
			case 'post' :
				return '<div class="badge badge-pill badge-dark">' . syntric_get_taxonomies_terms() . '</div>' . $lb;
				break;
		}
