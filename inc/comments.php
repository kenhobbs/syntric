<?php
	 ...
	 * @package syntric
	 */
	add_filter( 'comment_form_defaults', 'syntric_comment_form' );
	function syntric_comment_form( $args ) {
		global $post;
		...
		$args[ 'title_reply' ]          = __( 'Comment' );
		$args[ 'title_reply_to' ]       = __( 'Reply' );
		$args[ 'title_reply_after' ]    = '</h2>';
		$args[ 'comment_notes_before' ] = '';
		$args[ 'label_submit' ]         = 'Submit';
		$args[ 'must_log_in' ]          = sprintf( __( '<p><a href="%s">Login</a> to submit comments.</p>' ), wp_login_url( get_the_permalink() ) );
