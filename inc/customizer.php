<?php
	...
	function syn_customize_register( $wp_customize ) {
		...
		$wp_customize->add_section( 'syntric_theme_section_site_width', [ 'title'       => __( 'Site Width', 'syntric' ),
		                                                                  'capability'  => 'edit_theme_options',
		                                                                  'description' => __( 'Configure the site width', 'syntric' ),
		                                                                  'priority'    => 160,
		                                                                  //'panel'         => 'syntric_theme_panel'
		] );
		$wp_customize->add_section( 'syntric_theme_section_colors', [ 'title'       => __( 'Colors', 'syntric' ),
		                                                              'capability'  => 'edit_theme_options',
		                                                              'description' => __( 'Configure the site colors', 'syntric' ),
		                                                              'priority'    => 161,
		                                                              //'panel'         => 'syntric_theme_panel'
		] );
		/**
		 ...
		 * ) );
		 */
		$wp_customize->add_setting( 'syntric_theme_css', [ 'default'           => 'grey',
		                                                   'type'              => 'theme_mod',
		                                                   'sanitize_callback' => 'sanitize_text_field',
		                                                   'capability'        => 'edit_theme_options', ] );
		...
		$wp_customize->add_control(
			new WP_Customize_Control(
				...
				'syntric_container_type', [
					...
					'section'     => 'syntric_theme_section_site_width',
	...
	add_action( 'customize_register', function( $wp_customize ) {
		if ( isset( $wp_customize->nav_menus ) && is_object( $wp_customize->nav_menus ) ) {
			...
			//Remove all the filters/actions resiterd in WP_Customize_Nav_Menus __construct
			remove_filter( 'customize_refresh_nonces', [ $wp_customize->nav_menus,
			                                             'filter_nonces' ] );
			remove_action( 'wp_ajax_load-available-menu-items-customizer', [ $wp_customize->nav_menus,
			                                                                 'ajax_load_available_items' ] );
			remove_action( 'wp_ajax_search-available-menu-items-customizer', [ $wp_customize->nav_menus,
			                                                                   'ajax_search_available_items' ] );
			remove_action( 'customize_controls_enqueue_scripts', [ $wp_customize->nav_menus,
			                                                       'enqueue_scripts' ] );
			remove_action( 'customize_register', [ $wp_customize->nav_menus,
			                                       'customize_register' ], 11 );
			remove_filter( 'customize_dynamic_setting_args', [ $wp_customize->nav_menus,
			                                                   'filter_dynamic_setting_args' ], 10, 2 );
			remove_filter( 'customize_dynamic_setting_class', [ $wp_customize->nav_menus,
			                                                    'filter_dynamic_setting_class' ], 10, 3 );
			remove_action( 'customize_controls_print_footer_scripts', [ $wp_customize->nav_menus,
			                                                            'print_templates' ] );
			remove_action( 'customize_controls_print_footer_scripts', [ $wp_customize->nav_menus,
			                                                            'available_items_template' ] );
			remove_action( 'customize_preview_init', [ $wp_customize->nav_menus,
			                                           'customize_preview_init' ] );
			remove_filter( 'customize_dynamic_partial_args', [ $wp_customize->nav_menus,
			                                                   'customize_dynamic_partial_args' ], 10, 2 );
		}
	}, - 1 );
	/**
	...
	function syn_customize_preview_init() {
		wp_enqueue_script( 'syntric_customizer', get_template_directory_uri() . '/assets/js/customizer.js', [ 'customize-preview' ], '20170504', true );
	}