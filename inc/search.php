<?php
	 ...
	 */
	add_filter( 'pre_get_posts', 'syn_search_query' );
	function syn_search_query( $query ) {
		global $wp_query;
	...
	function syn_search_posts_distinct( $distinct, \WP_Query $q ) {
		if ( ! is_admin() && $q->is_search() && $q->is_main_query() ) {
			//remove_filter( 'posts_join', 'syn_search_posts_join' );
			$distinct = "DISTINCT";
		}
	 ...
	 */
	// Do not delete! The function is called by hook in pre_get_posts for searches
	function syn_search_posts_join( $join, \WP_Query $q ) {
