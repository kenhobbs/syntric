...
function syn_is_dev() {
	if ( $_SERVER[ 'SERVER_ADDR' ] == '127.0.0.1' || '::1' == $_SERVER[ 'SERVER_ADDR' ] || 'master.localhost' == $_SERVER[ 'HTTP_HOST' ] ) {
		return true;
	}
...
function syn_generate_login( $name ) {
...
function syn_get_taxonomies_terms() {
	global $post;
	$ret         = '';
	$taxes_terms = wp_get_object_terms( $post->ID, get_object_taxonomies( $post ), array( 'orderby' => 'term_group' ) );
